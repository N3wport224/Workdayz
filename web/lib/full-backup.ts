 ...
 * keys were restored. Only `workdayz.*` keys are ever written. */
export function restoreFullBackup(backup: FullBackup): number {
  ...
  for (const key of Object.keys(window.localStorage)) {
    if (key.startsWith("workdayz.")) window.localStorage.removeItem(key);
  }
  let restored = 0;
  for (const [key, value] of Object.entries(backup.entries)) {
    if (!key.startsWith("workdayz.") || typeof value !== "string") continue;
